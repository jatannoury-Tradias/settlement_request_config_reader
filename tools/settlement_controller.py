...
def settlement_controller(item,start_time,end_time):
    legs_to_be_settelled,trades,start_date,end_date = spaceship_controller.get_paginated_trades(client_id = item['client_id'],start_time=start_time,end_time=end_time)
    trades = list(set(trades))

    if legs_to_be_settelled == {} or trades == []:
        return item
    curr_client_addresses = spaceship_controller.get_client_addresses(item['client_id'])
    ...
    response = spaceship_controller.post_settlement_request(body)
...
def tfs_settlement_controller(item,tfs):
    legs_to_be_settelled = {}
    trades = []
    for tf in tfs:
        start_time,end_time = tf
        curr_legs_to_be_settelled,curr_trades,start_date,end_date = spaceship_controller.get_paginated_trades(client_id = item['client_id'],start_time=start_time,end_time=end_time)
