...
def configs_resolver(json_response, s3, env):
    ...
    for key, item in json_response.items():
        ...
        curr_client_token = client_tokens.get(
            get_client_legacy_id(client_spaceship_id=curr_client_spaceship_id, env=env), None)
        if curr_client_token == None:
            logger.log_warning(
                f"Client: {item['client_name']} with id:{item['client_id']} is not in the scraped tokens mapping")
            continue
        ...
        trigger_day = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"][
            current_date.weekday()]
        logger.log_info(f"Trigger Day:{trigger_day}")
        if item['frequency'] == "Daily":
            if current_date.weekday() == 0:
                if item['include_weekend'] == True:
                    start_time, end_time = get_72_hours_tf_from_limit(item['end_time'])
                    sr_data = settlement_controller(item, start_time, end_time, curr_client_token, env,
                                                    user_token=token, spaceship_id=curr_client_spaceship_id)
                    ...
                    if item['end_time'] == "12:00 AM":
                        ...
                        start_time, end_time = get_24_hours_tf_from_limit(item['end_time'],
                                                                          anchor=current_date - datetime.timedelta(
                                                                              days=3))
                ...
                if start_time == False or end_time == False:
                    continue
                sr_data = settlement_controller(item, start_time, end_time, client_token=curr_client_token, env=env,
                                                user_token=token, spaceship_id=curr_client_spaceship_id)
                json_response[key] = sr_data
            ...
            if item['day_limit'] == trigger_day:
                if item['include_weekend'] == True:
                    ...
                    sr_data = tfs_settlement_controller(item, [(start_time, left_limit), (right_limit, end_time)],
                                                        curr_client_token, env, user_token=token,
                                                        spaceship_id=curr_client_spaceship_id)
                    json_response[key] = sr_data
            else:
                continue
            ...
            if item['day_limit'] == "EOM":
                ...
                if item['include_weekend'] == True:
                    ...
                    settlement_controller(item, start_time.strftime('%Y-%m-%dT%H:%M:%S.%fZ'),
                                          current_date.strftime('%Y-%m-%dT%H:%M:%S.%fZ'), curr_client_token, env,
                                          user_token=token, spaceship_id=curr_client_spaceship_id)
                    ...
                    for frame in frames:
                        lower_bound = frame[0]
                        upper_bound = frame[1]
                        frame[0] = datetime.datetime(lower_bound.year, lower_bound.month, lower_bound.day, 0, 0,
                                                     0).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
                        frame[1] = datetime.datetime(upper_bound.year, upper_bound.month, upper_bound.day, 23, 59, 59,
                                                     999999).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
                    sr_data = tfs_settlement_controller(item, frames, env=env, client_token=curr_client_token,
                                                        user_token=token, spaceship_id=curr_client_spaceship_id)
                    json_response[key] = sr_data
